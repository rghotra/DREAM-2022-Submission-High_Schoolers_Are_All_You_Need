...
with open('../Training Code/test_sequences.txt') as handle:
    testing_seqs = pd.read_csv(handle, sep='\n', header=None).to_numpy()

seqs = []
for i in range(len(testing_seqs)):    
    seq, _ = testing_seqs[i][0].split('\t')
    seqs.append(seq)
testing_seqs = np.array(seqs)
